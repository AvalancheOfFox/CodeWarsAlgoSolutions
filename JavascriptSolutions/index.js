...
function meeting(s) {
  arr = s
    .toUpperCase()
    .split(';')
    .sort((name) => name.substring(name.indexOf(':') + 1, name.length));
  lastNames = arr.map(
    (name) =>
      '(' +
...
function iqTest(numbers) {
  ...
  return even.length < odd.length
    ? splitNum.indexOf(even[0]) + 1
    : splitNum.indexOf(odd[0]) + 1;
}
...
let addLetters = (...letters) =>
  String.fromCharCode(
    (letters.reduce((sum, l) => sum + l.charCodeAt() - 96, 0) % 26 || 26) + 96
  );
...
function nbYear(p0, percent, aug, p) {
  // if population is >= p, return count of loops we made
  let population = p0;
  let years = 0;
  while (p > population) {
